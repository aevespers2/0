...
This document defines a shared candidate contract between Repository `0` and Repository `1` for portable first-install security, device recovery, and evidence-backed remediation. It does not activate device-management authority, credentials, remote administration, remediation commands, or canonical-state mutation.

## Contract purpose

The contract allows Repository `0` to describe an observed device state and propose a bounded change while allowing Repository `1` to validate identity, policy, freshness, authority, and recovery conditions before any consequential operation is approved.
...
`0:proposal` is local staging only. It is not canonical state, approval, or authority.
...
Every envelope must carry the following identifiers or be rejected:
...
`device_id` must not rely only on hostname, IP address, Bluetooth name, user-visible label, or mutable serial aliases. The candidate identity record should separate:
...
Repository `1` must reject proposals that are ambiguous, overbroad, stale, wrong-device, unsupported-version, missing evidence, or unable to roll back safely.
...
Revocation messages must identify:
...
Emergency stop fails closed. Repository `0` must stop starting new consequential work and preserve evidence. Repository `1` must stop new capability issuance. There is no automatic unlock.
