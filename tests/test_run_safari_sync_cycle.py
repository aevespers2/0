...
def test_cycle_passes_safari_target_options(monkeypatch, tmp_path) -> None:
    ...
    watch_call = next(command for command in calls if any("watch_safari_dispatch_send.py" in item for item in command))
    assert "--target" in watch_call
    assert str(tmp_path / "FederationRelay" / "safari_target.json") in watch_call
    assert "--url" in watch_call
    assert "https://chatgpt.com/c/example" in watch_call
    extract_call = next(command for command in calls if any("extract_safari_ack.py" in item for item in command))
    assert "--target" in extract_call
    assert str(tmp_path / "FederationRelay" / "safari_target.json") in extract_call
    assert "--url" in extract_call
    assert "https://chatgpt.com/c/example" in extract_call
