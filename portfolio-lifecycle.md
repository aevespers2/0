...
QSO-GENOMES draft PR #12 opened on the canonical PR #2 lineage to remove active Aequitas and Socrates identities, retire `jacob_elias_redmond`, and transfer bounded seeker/review responsibility to `jacob_thomas_redmond`. The candidate is open, draft, and mechanically mergeable at exact head `85ab35b45a5819239902b6293ee4f5644450e98b`, based on PR #2 head `3cb0257149374f228e6bba483a12469719a194d2`; it changes 12 files with substantial deletion of active identity, binding, validator, sprite, and focused-test surfaces. This is an unapproved authority and compatibility-set migration, not acceptance evidence.

Exact-head QSO-GENOMES conformance run `29665460559` failed on Python 3.11, 3.12, and 3.13 during Draft 2020-12 genome/sprite schema validation. The deterministic contract suite and compatibility-manifest replay were skipped. PR #12 therefore cannot be merged into the canonical lineage or used to regenerate authoritative hashes until the validation failure is repaired, all remaining live legacy references are removed, affected digests are regenerated, negative migration tests pass, and provenance plus rollback are independently reviewed.

No reviewed default branch advanced after the prior lifecycle-plan commit. QuantumStateObjects `main` remains at unexpected consent-control head `15a337cb11737629aaae8704e4e8fd3a9366e0c2` while PR #7 remains open at `291d7419bf29a3d979762c4655c05a2a672c6f82`; the changed base, eight current P2 findings, and Issue #8 remain unresolved. QSO-GENOMES PR #2 remains canonical, open, draft, and non-mergeable, and its body still requests reconciliation against obsolete head `abda2ac987d39dd04ddc502cff8e0673a4f3de59`.
...
2. **QSO-GENOMES:** record one explicit architecture decision for PR #12's authority migration, repair and independently verify its failed exact-head conformance gate, then reconcile and freeze canonical PR #2 at one mergeable lineage with regenerated hashes, current provenance, and resolved/dispositioned findings. Separately approve or reject the repository-control and consent-control surfaces.
...
|---|---|---|
| `aevespers2/0` | `fc9dacc85d112786931866031e82bbcb54b87593` before this plan update | Planning baseline in progress |
...
| `aevespers2/QSO-GENOMES` | `3ede809df9a77c8e8fb38644b9d99eb51c44fbb8` | Automation/consent-expanded main; PR #2 and PR #12 blocked |
...
- **Architecture / acceptance / non-goals:** PR #2 is canonical, open, draft, and non-mergeable at `3cb0257149374f228e6bba483a12469719a194d2`; its body still names obsolete head `abda2ac987d39dd04ddc502cff8e0673a4f3de59`. Draft PR #12 is stacked directly on that lineage at `85ab35b45a5819239902b6293ee4f5644450e98b` and proposes an active authority migration by deleting Aequitas/Socrates/Jacob Elias records and promoting Jacob Thomas Redmond, but exact-head run `29665460559` failed schema/sprite validation on Python 3.11–3.13 and skipped the deterministic and manifest stages. Current findings include compatibility-set contract defects; workflow ref handling, failure retention, conformance coverage, receipt-digest validation, already-current retries, timeout reporting, reachable provenance, fork repair bases, workflow routing/context, and reconciliation branch uniqueness; plus incomplete legacy-reference removal, digest regeneration, negative migration coverage, authority-semantics review, and rollback documentation. Require one explicit authority-migration decision, one mergeable immutable candidate head, successful exact-head suite, resolved/dispositioned findings, explicit digest scopes, immutable oversight validation, exact four-genome set, negative fixtures, provenance-preserving reconciliation, downstream acceptance, and separate approval records for automation and consent-control surfaces. No executable QSO behavior, credentials, payments, autonomous policy change, or automation-derived acceptance.
- **Release / version / changelog / artifacts:** blocked `0.1.0-alpha.1`; require schemas/genomes/protocol/migration/oversight artifacts, compatibility manifest and regenerated per-object/set hashes, fixtures, identity-migration and reconciliation records, review map, exact-head/clean-checkout/downstream reports, failed-run disposition, consent-gate report, checksums, changelog, SBOM/dependency record where applicable, and provenance.
- **Deploy / health / observability / rollback / validation:** no compatibility-set publication or runtime deployment. Treat merged workflows and consent controls as separate repository-control planes: health must expose actor authorization, event source, exact source/base heads, branch and draft-PR effects, duplicate suppression, timeout/failure artifacts, schema/identity/gate findings, and no execution of untrusted candidate code with write authority. Rollback preserves the PR #12 and PR #2 histories and failed evidence, restores the last approved authority/control configuration through reviewed change control, and proves compatibility artifacts and hashes were not silently rewritten.
